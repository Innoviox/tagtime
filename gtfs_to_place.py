...
class GTFSParser:
    ...
    def parse_trips(self):
        ...
        with open(f"{self.base_path}/stop_times.txt") as f:
            ...
            for row in tqdm.tqdm(list(reader)):
                ...
                if parent:
                    self.trips[row["trip_id"]]["stations"][parent] = time_to_seconds(
                        row["arrival_time"]
                    )
                else:
    ...
    def to_json(self):
        ...
        return json.dumps(
            {"stations": self.stations, "routes": self.routes, "trips": self.trips}
        )


if __name__ == "__main__":
    open("places/dc.json", "w").write(
        GTFSParser(
            "transitland/f-dqc-wmata~rail-17682fd6de41fac6919edc1f433c8fc1f4aab3a8",
            "STN",
            "METRORAIL STATION",
        ).to_json()
    )
